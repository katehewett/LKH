...
mdates2 = pd.date_range(start='2012-12-31',end='2024-01-01', freq='Y')
...
for sn in sn_list:
    ...
    fn_out = posixpath.join(out_dir, ('new1_' +sn + '.png'))
    ...
    plt.plot(ltime,lo_ds.SA,color = 'dodgerblue',marker='none',
             linestyle='-',linewidth=3,alpha=0.6,markeredgecolor='none',label ='LO')
    plt.plot(ot,obs_ds.SA.values,color = 'navy',marker='none',
    ...
    ax0.set_xticks(mdates2)
    plt.grid(axis = 'x')
    ax0.set_title((sn+': surface LO model output data; surface data obs'))
    ax0.set_ylabel('SA g/kg')
    ax0.legend(loc="best",frameon=False)
    
    ax1 = plt.subplot2grid((3,1),(1,0),colspan=1,rowspan=1)
    plt.plot(ltime,lo_ds.CT,color = 'dodgerblue',marker='none',
             linestyle='-',linewidth=3,alpha=0.6,markeredgecolor='none',label ='LO')
    plt.plot(ot,obs_ds.CT.values,color = 'navy',marker='none',
             linestyle='-',linewidth=3,alpha=0.6,markeredgecolor='none',label ='obs')
    ax1.set_xticklabels([])
    ax1.set_ylim([6,22])
    ax1.set_yticks(np.arange(6,23,4), minor=False)
    ax1.set_xlim([mdates1[0],mdates1[-1]])
    ax1.set_xticks(mdates2)
    plt.grid(axis = 'x')
    ax1.set_ylabel('CT deg C')
    
    ax2 = plt.subplot2grid((3,1),(2,0),colspan=1,rowspan=1)
    plt.plot(ltime,lo_ds['DO (uM)'].values,color = 'dodgerblue',marker='none',
             linestyle='-',linewidth=3,alpha=0.6,markeredgecolor='none',label ='LO')
    plt.plot(ot,obs_ds['DO (uM)'].values,color = 'navy',marker='none',
             linestyle='-',linewidth=3,alpha=0.6,markeredgecolor='none',label ='obs')
    #ax2.set_xticklabels([])
    ax2.set_ylim([100,520])
    ax2.set_yticks(np.arange(100,521,200), minor=False)
    ax2.set_xlim([mdates1[0],mdates1[-1]])
    ax2.set_xticks(mdates2)
    ax2.set_xticks(mdates2)
    plt.grid(axis = 'x')
...
for sn in sn_list:
    ...
    obs_fn = posixpath.join(obs_in_dir, (sn+'_daily_Oag.nc'))
    model_fn = posixpath.join(model_in_dir, ('LO_'+sn + '_surface.nc'))
    fn_out = posixpath.join(out_dir, ('Cnew2_' +sn + '.png'))
    ...
    plt.plot(ltime,lo_ds['pH_total'].values,color = 'dodgerblue',marker='none',
             linestyle='-',linewidth=3,alpha=0.6,markeredgecolor='none',label ='LO')
    plt.plot(ot,obs_ds['pH'].values,color = 'navy',marker='none',
             linestyle='-',linewidth=3,alpha=0.6,markeredgecolor='none',label ='obs')
    plt.plot(ot,obs_ds['new_pH'].values,color = 'green',marker='none',
             linestyle='-',linewidth=3,alpha=0.6,markeredgecolor='none',label ='new_pH/obs calc')
    ax3.set_title((sn+': surface LO model output data; surface data obs'))
    ax3.set_xticklabels([])
    ax3.set_ylim([7.2,8.6])
    ax3.set_yticks(np.arange(7.2,8.7,.4), minor=False)
    ax3.set_xlim([mdates1[0],mdates1[-1]])
    ax3.set_xticks(mdates2)
    plt.grid(axis = 'x')
    ax3.set_ylabel('pH total')
    
    ax4 = plt.subplot2grid((3,1),(1,0),colspan=1,rowspan=1)
    plt.plot(ltime,lo_ds.ARAG,color = 'dodgerblue',marker='none',
             linestyle='-',linewidth=3,alpha=0.6,markeredgecolor='none',label ='LO')
    plt.plot(ot,obs_ds['ARAG'].values,color = 'navy',marker='none',
             linestyle='-',linewidth=3,alpha=0.6,markeredgecolor='none',label ='obs')
    ax4.set_xticklabels([])
    ax4.set_ylim([0.5,4.5])
    ax4.set_yticks(np.arange(0.5,4.6,1), minor=False)
    ax4.set_xlim([mdates1[0],mdates1[-1]])
    ax4.set_xticks(mdates2)
    plt.grid(axis = 'x')
    ax4.set_ylabel('O ARAG')
    
    ax5 = plt.subplot2grid((3,1),(2,0),colspan=1,rowspan=1)
    plt.plot(ltime,lo_ds.pCO2,color = 'dodgerblue',marker='none',
             linestyle='-',linewidth=3,alpha=0.6,markeredgecolor='none',label ='LO')
    plt.plot(ot,obs_ds['pCO2_sw'].values,color = 'navy',marker='none',
             linestyle='-',linewidth=3,alpha=0.6,markeredgecolor='none',label ='obs')
    ax5.set_ylim([50,650])
    ax5.set_yticks(np.arange(50,651,100), minor=False)
    ax5.set_xlim([mdates1[0],mdates1[-1]])
    ax5.set_xticks(mdates2)
    plt.grid(axis = 'x')
