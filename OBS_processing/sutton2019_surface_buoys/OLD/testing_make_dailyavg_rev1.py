...
Take w/ a grain o salt:
CHL is werid, there are often 2 samples/day ? Taking that for now. And Turb data is sparse!
'''
...
for vn in var_list:   
    ...
    # this is long for plotting ease, want to seperate deployment chunks
    ...
    plt.plot(dt,ds[vn].values,color = 'black',marker='o',linestyle='none',linewidth=3,label ='SA')
    ...
    if (str(vn)!='CHL') and (str(vn)!='TURB'):
        condition1 = daily_stats['count']<7
        condition2 = time_helper['DT']<timedelta(hours=18) # ideally want times where: 0:17 to 21:17 UTC but will take 18h
    ...
    mdates0 = pd.date_range(start=tstart,end=tend, freq='6M')
    ...
    if 'ds_daily' in locals(): #dataset exists
        ...
        attrs={'Station Name':sn_name_dict[sn],'lon':ds.lon,'lat':ds.lat,
                'Depth':'surface ~0.5m', 'Source file':str(fn_in),
                'data processed': 'Daily Averages @ 12UTC'})
        
        ds_daily[str(vn)] = xr.DataArray(df_resampled['daily_avg'].values,dims=('datetime_utc'),
                attrs={'units':ds[str(vn)].attrs['units'], 'long_name':ds[str(vn)].attrs['long_name'], 
                'depth': ds[str(vn)].attrs['depth']})
    else: 
        print('error with vars @ step: '+str(vn))
        sys.exit() 
