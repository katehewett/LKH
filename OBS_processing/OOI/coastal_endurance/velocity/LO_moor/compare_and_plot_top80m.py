...
vbar1 = np.nanmean(v1[:,17:NZ],axis=1)
vbar2 = np.nanmean(v2[:,17:NZ],axis=1)
ubar1 = np.nanmean(u1[:,17:NZ],axis=1)
ubar2 = np.nanmean(u2[:,17:NZ],axis=1)
...
ax1.set_title(moor + ' depth averaged velocity (top ~85m)')
...
ax2.set_yticks(yy)
ax2.set_ylabel('mean u m/s')

ax2.set_xlim(datetime(2017,1,1),datetime(2018,1,1))

figname = 'LO_top80m_comparision_' + moor+'_'+str(thisyr)+'.png'
