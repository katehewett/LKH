...
if loco == 'surfacebuoy':
    fn_in = '/Users/katehewett/Documents/LKH_data/OOI/CE/coastal_moorings/'+moor+'/velocity/'+loco+'/daily/'+moor+'_surfacebuoy_VELPTA_DAILY.nc'       
elif loco == 'nsif':
    fn_in = '/Users/katehewett/Documents/LKH_data/OOI/CE/coastal_moorings/'+moor+'/velocity/'+loco+'/daily/'+moor+'_nsif_VELPTA_DAILY.nc'
...
for ydx in range(0,numyrs): 
    ...
    if np.shape(mdx)[1] < 365:
        ...
        if dt[-1] == dti[-1]: 
            ...
            u = np.concatenate((nanarray,ds.u.values[mdx]), axis=0)
            v = np.concatenate((nanarray,ds.v.values[mdx]), axis=0)
            w = np.concatenate((nanarray,ds.w.values[mdx]), axis=0)
            z = np.concatenate((zarray,ds.w.values[mdx]), axis=0)
        else:
            j = np.where(t2==dti)
            ...
            u = np.concatenate((ds.u.values[mdx],nanarray), axis=0)
            v = np.concatenate((ds.v.values[mdx],nanarray), axis=0)
            w = np.concatenate((ds.w.values[mdx],nanarray), axis=0)
            z = np.concatenate((ds.z.values[mdx],zarray), axis=0)
        ...
        ds1['z'] = (('ocean_time'), z, {'units':'m', 'long_name':'altitude from OOI; depth below surface'})
        ds1['u'] = (('ocean_time'), u, {'units':'m.s-1', 'long_name': 'Daily avg OOI Eastward Sea Water Velocity','positive':'eastward',
        ...
        ds1['v'] = (('ocean_time'), v, {'units':'m.s-1', 'long_name': 'Daily avg OOI Northward Sea Water Velocity','positive':'northward',
        ...
        ds1['w'] = (('ocean_time'), w, {'units':'m.s-1', 'long_name': 'Daily avg Upward Sea Water Velocity','positive':'upward',
        ...
        fn_name = str(moor) + '_' + loco + '_VELPTA_daily_'+str(thisyr)+'.01.01_'+str(thisyr)+'.12.31.nc'
