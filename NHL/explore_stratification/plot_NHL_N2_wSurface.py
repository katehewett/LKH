...
scb = plt.gcf().colorbar(isig, ticks = [21,22,23,24,25], location='bottom', pad = 0.05, label='surface')

## Create salt plot
#slevels = np.arange(24,34.75,0.25)
#axnum = 0
##isalt = plt.gcf().axes[axnum].pcolormesh(X,Y,S,cmap=cmc.roma_r, vmin = np.min(slevels), vmax = np.max(slevels))
#isalt = plt.gcf().axes[axnum].contourf(X,Y,S,slevels,cmap=cmc.roma_r)
#plt.gcf().axes[axnum].invert_yaxis()
#plt.gcf().axes[axnum].set_title('LO/NHL_transect: SA')

#scb = plt.gcf().colorbar(isalt, ticks = [24, 26, 28, 30, 32, 34.5], location='bottom', pad = 0.05, label='SA surface')

# Create salt plot
slevels2 = np.arange(31.5,34.75,0.25)
axnum = 1
smap=cmc.roma_r.with_extremes(under='Black')
#isalt = plt.gcf().axes[axnum].pcolormesh(X,Y,S,cmap=cmc.roma_r, vmin = np.min(slevels), vmax = np.max(slevels))
isalt2 = plt.gcf().axes[axnum].contourf(X,Y,S,slevels2,cmap=smap,extend="min")
plt.gcf().axes[axnum].invert_yaxis()
plt.gcf().axes[axnum].set_title('SA')

scb = plt.gcf().colorbar(isalt2, ticks = [31.5, 32.5, 33.5, 34.5], location='bottom', pad = 0.05, label='SA surface')
...
# Create TEMP plot
tlevels = np.arange(7,18.75,0.25)
axnum = 2
itemp = plt.gcf().axes[axnum].contourf(X,Y,T,tlevels,cmap=cmc.roma_r)
plt.gcf().axes[axnum].invert_yaxis()
plt.gcf().axes[axnum].set_title('CT')

tcb = plt.gcf().colorbar(itemp, ticks = [7,9,11,13,15,17,18.5], location='bottom', pad = 0.05, label='CT surface')
tcb.set_ticklabels(['7', '9','11','13','15','17','18.5'])
