...
fnn = 'NHL_bio_2014.01.01_2019.12.31'
...
h = np.expand_dims(ds.h.values[ilat,ilon],axis=0)
mask_rho = np.expand_dims(ds.mask_rho[ilat,ilon],axis=0)

#z_w = np.expand_dims(ds.z_w[:,:,idx,:],axis=2) 
#z_rho = np.expand_dims(ds.z_rho[:,:,idx,:],axis=2)  
oxygen = np.expand_dims(ds.oxygen[:,:,ilat,ilon],axis=2)  
NO3 = np.expand_dims(ds.NO3[:,:,ilat,ilon],axis=2)  
NH4 = np.expand_dims(ds.NH4[:,:,ilat,ilon],axis=2)  
phyto = np.expand_dims(ds.phytoplankton[:,:,ilat,ilon],axis=2) 
zoop = np.expand_dims(ds.zooplankton[:,:,ilat,ilon],axis=2) 
...
# plot temperature
axnum = 0
tmin = math.floor(np.min(oxygen.values))
tmax = math.ceil(np.max(oxygen.values))
tlevels = np.arange(tmin,tmax,0.1)

iCT = plt.gcf().axes[axnum].contourf(XX,z_rho,oxygen,tlevels,cmap=cmc.roma_r)
