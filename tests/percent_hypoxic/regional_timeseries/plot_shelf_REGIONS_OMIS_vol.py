...
#fn = fn_i / 'hypoxic_volume' / 'combined_2017_2022' / 'hypoxic_volumes_2017_2022.nc'
#fn2 = fn_i / 'bottom_layer' / 'grid_extractions_2017.01.01_2022.12.31' / 'GZinfo_2017.01.01_2022.12.31.nc'
...
if (calc_volumes == True & plot_regions == True):
    ...
    hypV = DA * hyp_dz        # the hyp volume on each rho point
    
    #hypVm = np.nan * np.ones((NT, NR, NC))    # initialize a bunch of vars for saving stuff 
    hypVT = np.nan * np.ones((NT,NMASK))
    frac_hypV = np.nan * np.ones((NT,NMASK))
    ...
    for t in range(NT):
        ...
        for mm in range(NMASK):
            hypVT[t,mm] = np.nansum(hvt[mask_dict[lat_list[mm]]])
            frac_hypV[t,mm] = (hypVT[t,mm]/VT) * 100
...
if (calc_volumes == True) & (plot_regions == True):     
    ...
    ZZ = hypVT[:,3]*0
    ax1.fill_between(mdt,ZZ,hypVT[:,3]/1e9, color=Rcolors[3], alpha=0.6)
