...
mdates1 = pd.date_range(start='2017-04-01',end='2017-12-01', freq='MS')
mdates2 = pd.date_range(start='2017-04-01',end='2017-12-01', freq='16D')
...
for sn in sn_list:
    ...
    if os.path.isfile(in_fn):
        ...
        # Create salt plot
        axnum = 0
        isalt = plt.gcf().axes[axnum].pcolormesh(xr,yr,Csalt,cmap=cmc.roma_r, vmin = 31.5, vmax = 34)
        plt.gcf().axes[axnum].set_title(sn+': 2017')
        
        fig.colorbar(isalt,shrink=0.75,ticks=[31.5,32.5,33.5,34],label='salinity P')
        ...
        # Create temp plot
        axnum = 1
        itemp = plt.gcf().axes[axnum].pcolormesh(xr,yr,Ctemp,cmap=cmc.roma, vmin = 6, vmax = 12)
        
        fig.colorbar(itemp,shrink=0.75,ticks=[6,7,8,9,10,11,12],label='temp deg.C')
        plt.gcf().axes[axnum].set_ylabel('depth m')
        ...
        # Create oxygen plot
        axnum = 2
        ioxygen = plt.gcf().axes[axnum].pcolormesh(xr,yr,Coxy,cmap=cmc.hawaii, vmin = 0, vmax = 5)
        
        fig.colorbar(ioxygen,shrink=0.75,ticks=[0,1,2,3,4,5],label='(DO ml/L)')
        plt.gcf().axes[axnum].set_ylabel('depth m')
        ...
        fig.colorbar(imix,shrink=0.75,ticks=[-12, -9, -6, -3, -1],label='log Ks m2/s')
        plt.gcf().axes[axnum].set_ylabel('depth m')
