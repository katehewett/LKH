...
run panel_plot_DO -gtx cas7_t0_x4b -y0 2017 -y1 2023 -job OCNMS_CE_moorings -moor CE042 
...
fn_i = Ldir['LOo'] / 'extract' / args.gtagex / 'moor' / args.job_type 


fn_o = Ldir['parent'] / 'LKH_output' / 'WOAC' / 'moor_pannel_plots' / args.gtagex / 'moor' / args.job_type 
fno_name = args.mname + '_moor_DO_'+str(args.ys0)+'_'+str(args.ys1)+'.png'
...
for ydx in range(0,numyrs):
    print('plotting'+str(yr_list[ydx]))
    thisYR = yr_list[ydx]
    ...
    ax2 = plt.subplot2grid((7,1), (ydx,0), colspan=1)  # T 
    pcm2 = ax2.pcolormesh(X, Y, rho, cmap = smap, shading='gouraud')#,vmin=0, vmax=8) #, norm=norm)
    cbar_aou = fig1.colorbar(pcm2) #, ticks=[-120, -60, 0, 60, 120, 180, 240, 300])
    cbar_aou.set_label('DO ml/L')
