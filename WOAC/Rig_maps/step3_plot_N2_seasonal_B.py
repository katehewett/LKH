...
figname = 'N2_seasonal_median_'+str(args.ys0)+'_'+str(args.ys1)+'_OA_indicators.png'
...
# plot data 
cpw = axw.pcolormesh(W['lon_rho'], W['lat_rho'], np.log10(W['p50']),vmin=-5,vmax=-2,cmap=cm.roma_r)
cbaxes = inset_axes(axw, width="5%", height="40%", loc='lower right')
fig.colorbar(cpw, cax=cbaxes, location='left', orientation='vertical',label = 'log10 N2')

cpsp = axs.pcolormesh(SP['lon_rho'], SP['lat_rho'], np.log10(SP['p50']),vmin=-5,vmax=-2,cmap=cm.roma_r)
cbaxes = inset_axes(axs, width="5%", height="40%", loc='lower right')
fig.colorbar(cpsp, cax=cbaxes, location='left', orientation='vertical',label = 'log10 N2')

cpsu = axsu.pcolormesh(SU['lon_rho'], SU['lat_rho'], np.log10(SU['p50']),vmin=-5,vmax=-2,cmap=cm.roma_r)
cbaxes = inset_axes(axsu, width="5%", height="40%", loc='lower right')
fig.colorbar(cpsu, cax=cbaxes, location='left', orientation='vertical',label = 'log10 N2')

cpf = axf.pcolormesh(F['lon_rho'], F['lat_rho'], np.log10(F['p50']),vmin=-5,vmax=-2,cmap=cm.roma_r)
cbaxes = inset_axes(axf, width="5%", height="40%", loc='lower right')
fig.colorbar(cpf, cax=cbaxes, location='left', orientation='vertical',label = 'log10 N2')
