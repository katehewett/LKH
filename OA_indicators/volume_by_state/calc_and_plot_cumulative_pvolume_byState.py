"""
Plots cumulative shelf volume for each State (WA and Oregon) 
at Oag<1 and <0.5 


example calls:  
run calc_and_plot_cumulative_pvolume_byState -mvar ARAG1
run calc_and_plot_cumulative_pvolume_byState -mvar ARAG05
"""
# imports
...
# shade states
ax.scatter(xrho*mask_dict['WA'], yrho*mask_dict['WA'], s=2, c = '#B79346') # yellow 
ax.scatter(xrho*mask_dict['OR'], yrho*mask_dict['OR'], s=2, c = '#294066') # blue 

ax1 = plt.subplot2grid((2,3), (0,1), colspan=1, rowspan = 1)  # top
...
RW = np.full(numyrs, np.nan)
RO = np.full(numyrs, np.nan)
